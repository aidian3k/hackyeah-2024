...
public class InstitutionService {
    ...
    public InstitutionOutDTO handleInstitutionSearch(
        InstitutionSearchFiltersDTO institutionSearchFiltersDTO
    ) throws JsonProcessingException {
        List<InstitutionOutDTO.SingleInstitution> singleInstitutions =
            institutionClient
                .handleGetInstitutionRequest(
                    objectMapper.writeValueAsString(institutionSearchFiltersDTO)
                )
                .getInstitutions()
                .stream()
                .map(institutionInfo ->
                    new InstitutionOutDTO.SingleInstitution(
        ...
        singleInstitutions.forEach(singleInstitution -> {
            if (!institutionsRepository.existsById(singleInstitution.id())) {
                institutionsRepository.save(
                    Institution
                        .builder()
