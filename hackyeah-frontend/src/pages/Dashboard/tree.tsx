...
const Tree = ({}) => {
  ...
  const TreeNode: React.FC<{ node: TreeNode; path: string[]; level: number }> = ({ node, path, level }) => {
    ...
    if (singleChild) {
      ...
      return <TreeNode node={node[childName]} path={[...path, childName]} level={level + 1} />;
    }
    ...
    return (
      <div className="ml-4">
        <div className="flex items-center py-1">
          ...
          {hasChildren ? (
            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={toggleExpand} aria-label={isExpanded ? 'Collapse' : 'Expand'}>
        ...
        {isExpanded && hasChildren && (
          <div className="ml-6">
            {Object.entries(node).map(([childName, childNode]) => (
              <TreeNode key={childName} node={childNode} path={[childName]} level={level + 1} />
            ))}
          </div>
        )}
      </div>
